...
#%%
stopwords = pd.read_csv('./utils_data/vietnamese-stopwords.txt', header = None)
stopwords_list = stopwords[0].to_list()

#%%
with open('./utils_data/vietnamese-stopwords.txt', 'r' , encoding='utf-8') as f:
...
def replace_acronym(text, dict_replace):
...
def clean_data(text):
    ...
    text = replace_acronym(text, dict_replace)
...
def tokenization(text):
    ...
    tokens = [value[0] for value in pos_tag(text) if (value[1] == 'N' or value[1] == 'Np')]
    clean_tokens = []
    for word in tokens:
